    ...
    for idx, agent_2 in enumerate(agents[:-1]):
        ...
        for p1, p2 in itertools.permutations((agent_1, agent_2)):
            ...
            for _ in range(num_matches):
                score_1, score_2 = play_match(p1.player, p2.player)
                round_score_1 += score_1
                round_score_2 += score_2
            counts[p1] += round_score_1
            counts[p2] += round_score_2
            total += round_score_1 + round_score_2
            print("\tResult: {} vs. {} - {} to {}".
                  format(p1.name, p2.name, int(round_score_1), int(round_score_2)))

        wins += counts[agent_1]

        print("\n Final Result: {} to {}".format(int(counts[agent_1]),
                                          int(counts[agent_2])))
...
def main():
    ...
    test_agents = [Agent(CustomPlayer(), "Agent_1"),
                   Agent(CustomPlayer(), "Agent_2")]
